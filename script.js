...
// ── LEVEL MODE SYSTEM (sonsuz, 12 farklı tip)
// 0=Normal park
// 1=Otopark (çok katlı)
// 2=Şerit takip
// 3=L-Park (paralel)
// 4=Geri giriş
// 5=Çoklu park (2 spot)
// 6=Hız limiti (çok yavaş park)
// 7=Karanlık park (düşük görüş)
// 8=Zigzag şerit
// 9=Dar otopark (çok dar koridor)
// 10=VIP park (belirli rotadan geç)
// 11=Fırtına (kaygan zemin)
...
// Seviye → mod eşleme (sonsuz, çeşitli pattern)
function getLevelMode(l){
  // Eğer drift game mode seçildiyse her zaman drift
  if(GAME_MODE==='drift') return 12;
  // İlk 5 seviye tanıtım
  if(l===1)return 0;
  if(l===2)return 3;
  if(l===3)return 2;
  if(l===4)return 1;
  if(l===5)return 4;
  // Park modunda drift seviyesi yok
  // Sonrası cycling pattern - ama tekrar etmemesi için offset
  const patterns=[
...
function cfg(l){
  ...
  // Park yeri küçülme faktörü: 1.75'ten başlar, seviye 30'da araç boyutuna iner (1.0)
  // Her 5 seviyede bir %8 küçülür, minimum araç boyutu (1.0)
  const spotShrink=Math.max(1.0, 1.75 - tier*0.075);
  const base={
    time:Math.max(20,70-tier*5-(l%4)*2),
    sm:Math.max(1.0,spotShrink),       // park yeri boyut çarpanı (küçülür)
    mf:3.0+tier*.25,
    obs:2+tier*2+(l%4),
  ...
  if(mode===12){
    // Drift modu: süre yok, serbest alan
    base.time=999;
    base.obs=1+tier;
    base.npc=0;
    base.needKey=false;
...
function inSpot(){
  if(!PS)return false;
  if(!cfg(LVL).needKey||KEYS_COLLECTED>=cfg(LVL).numKeys){
    return rcorners(car.x,car.y,car.angle,CW,CH).every(c=>c.x>PS.x&&c.x<PS.x+PS.w&&c.y>PS.y&&c.y<PS.y+PS.h);
  }
  return false;
}
function inSpot2(){
  if(!PS2)return false;
  return rcorners(car.x,car.y,car.angle,CW,CH).every(c=>c.x>PS2.x&&c.x<PS2.x+PS2.w&&c.y>PS2.y&&c.y<PS2.y+PS2.h);
}
...
let LANE_WIDTH=52;
let laneScore=0;
let PS2=null; // second park spot for mode 5
...
let CARPARK_WALLS=[];
let CARPARK_SPOTS=[];
let CARPARK_TYPE=0; // 0=normal, 1=dar, 2=çok katlı
